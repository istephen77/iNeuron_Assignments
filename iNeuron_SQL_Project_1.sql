...
SELECT AMOUNT, (EXTRACT(YEAR FROM TRANSACTION_DATE)*12 + EXTRACT(MONTH FROM TRANSACTION_DATE)), COUNT, 
AMOUNT - 5*(EXTRACT(YEAR FROM TRANSACTION_DATE)*12 + EXTRACT(MONTH FROM TRANSACTION_DATE) + 1 - COUNT) AS BALANCE
FROM (SELECT SUM(AMOUNT) AS AMOUNT, MAX(TRANSACTION_DATE), MIN(TRANSACTION_DATE) AS TRANSACTION_DATE 
FROM TRANSACTIONS T) T 
CROSS JOIN
(SELECT COUNT(*) AS COUNT
	FROM (SELECT EXTRACT(MONTH FROM TRANSACTION_DATE) AS YYYYMM, COUNT(*) AS COUNT, SUM(AMOUNT) AS AMOUNT
		FROM TRANSACTIONS t
		WHERE AMOUNT < 0
		GROUP BY YYYYMM
